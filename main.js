// Load console filter first to suppress unwanted errors
require('./utils/consoleFilter');
...
// Enhanced error handlers for Render
process.on('unhandledRejection', (reason, promise) => {
  if (reason && (
    ...
    reason.toString().includes('getThreadInfoGraphQL') ||
    reason.toString().includes('formatThreadGraphQLResponse') ||
    reason.toString().includes('Invalid response data') ||
    reason.toString().includes('Cannot read properties of undefined') ||
    reason.toString().includes('removeUserFromGroup') ||
    reason.toString().includes('Theme loading failed') ||
    reason.message?.includes('status code 401') ||
    reason.message?.includes('status code 404') ||
    reason.message?.includes('status code 500')
  )) {
    return; // Silently ignore common errors
  }
  
  // Log errors for Render monitoring
  if (process.env.RENDER) {
    console.log(`🔄 Render Error Handled: ${new Date().toISOString()}`);
  }
  
  // Only log critical errors
  if (reason.toString().includes('CRITICAL') || reason.toString().includes('FATAL')) {
    logger.log(`Critical Error: ${reason}`, "ERROR");
  }
});

process.on('uncaughtException', (error) => {
  if (error.code === 'ENOENT' || 
      error.message?.includes('getThreadInfoGraphQL') ||
      error.message?.includes('Cannot read properties') ||
      error.message?.includes('path') && error.message?.includes('cache')) {
    return; // Silently ignore common errors
  }
...
try {
  ...
  if (isRender) {
    ...
    setInterval(() => {
      console.log(`🔄 Render Keep-Alive: ${new Date().toISOString()}`);
    }, 300000); // Every 5 minutes
    
    logger.log(`🚀 Render detected - PORT: ${process.env.PORT}`, "RENDER");
    logger.log(`🌐 External URL: ${process.env.RENDER_EXTERNAL_URL}`, "RENDER");
  }
  
  webServer.start();
  logger.log("✅ Web server initialized successfully", "WEBSERVER");
} catch (error) {
  logger.log(`❌ Web server failed: ${error.message}`, "WEBSERVER");
}
...
// Initialize Render keep-alive system
try {
  const renderKeepAlive = require('./utils/renderKeepAlive.js');
  renderKeepAlive.start();
  logger.log("Render keep-alive system initialized", "RENDER");
} catch (error) {
  logger.log(`Render keep-alive initialization failed: ${error.message}`, "RENDER");
}
