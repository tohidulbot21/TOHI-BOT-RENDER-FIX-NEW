...
module.exports = function ({ api }) {
  ...
  return async (event) => {
    try {
      ...
      if (event.messageID) {
        const messageKey = `${event.threadID}-${event.messageID}`;
      ...
      if (!isAdmin && event.threadID && event.threadID !== event.senderID) {
        ...
        if (!Groups.isApproved(event.threadID)) {
          // Silent block for unapproved groups
          return;
        }
      }
      ...
      switch (event.type) {
        ...
        case "message_unsend":
          try {
            // Execute command handlers without logging here
            // All command logging is now handled in handleCommand.js
