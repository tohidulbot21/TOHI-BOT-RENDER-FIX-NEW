  ...
  try {
    ...
    if (fb_dtsg && jazoest) {
      ...
      try {
        ...
        utils.warn("Error reading existing fb_dtsg data, creating new file");
        existingData = {};
      }
      existingData[UID] = {
        fb_dtsg,
...
function buildAPI(html, jar) {
  ...
  cron.schedule(
    "0 0 * * *",
    () => {
      const fbDtsgPath = "includes/database/data/fb_dtsg.json";
      
      if (!fs.existsSync(fbDtsgPath)) {
        utils.warn("fb_dtsg.json file not found, skipping refresh");
        return;
      }
      
      const fbDtsgData = JSON.parse(
        fs.readFileSync(fbDtsgPath, "utf8"),
      );
      try {
        if (fbDtsgData && fbDtsgData[userID]) {
          ...
          utils.warn(`No fb_dtsg data found for user ${userID}.`);
        }
      } catch (error) {
        utils.error(`Error in fb_dtsg refresh cron job: ${error.message}`);
      }
    },
    {
