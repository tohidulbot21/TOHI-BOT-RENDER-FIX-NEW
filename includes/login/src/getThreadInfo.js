	...
	return function getThreadInfoGraphQL(threadID, callback) {
		...
		defaultFuncs
			...
			.then(function (resData) {
				if (!resData) {
					throw new Error("No response data received from GraphQL API");
				}
				if (resData.error) {
					throw resData;
				}
				...
				for (let i = resData.length - 2; i >= 0; i--) {
					if (resData[i] && Object.keys(resData[i]).length > 0) {
						const responseKey = Object.keys(resData[i])[0];
						const responseData = resData[i][responseKey];
						if (responseData && responseData.data) {
							...
							console.error(`getThreadInfoGraphQL: Invalid response data at index ${i}`);
						}
					}
				}
